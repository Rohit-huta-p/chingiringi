...
function page({ code, appUrl, storeUrl, webUrl }) {
...
.p{background:#6d28d9;color:#fff}.s{background:#1E293B;color:#cbd5e1}.code{font-size:22px;letter-spacing:2px;font-weight:800;margin:8px 0 24px}
.web{margin-top:16px}.web a{color:#94a3b8;font-size:13px;text-decoration:none}</style>
</head><body>
...
<p class="web"><a href="${webUrl}">or continue on web →</a></p>
<script>
...
router.get('/:code', (req, res) => {
  try {
    ...
    const webBase = process.env.WEB_APP_URL || 'https://chingiringi.com';
    const webUrl = `${webBase}/?ref=${code}`;
    const storeUrl = pickStoreUrl(req.headers['user-agent'], { ...STORES(), web: webUrl });
